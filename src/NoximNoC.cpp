...
void NoximNoC::buildMesh()
{
    ...
    if (DEFAULT_MESH_DIM_Z == 1) // set all vertical bits to zero for all nodes
    {
        for (int i1 = 0; i1 < NoximGlobalParams::mesh_dim_x; i1++)
            for (int j1 = 0; j1 < NoximGlobalParams::mesh_dim_y; j1++)
            {
                t[i1][j1][0] -> r -> Nu = false;
                t[i1][j1][0] -> r -> Eu = false;
                t[i1][j1][0] -> r -> Wu = false;
                t[i1][j1][0] -> r -> Su = false;
                
                t[i1][j1][0] -> r -> Nd = false;
                t[i1][j1][0] -> r -> Nd = false;
                t[i1][j1][0] -> r -> Nd = false;
                t[i1][j1][0] -> r -> Nd = false;
            }
